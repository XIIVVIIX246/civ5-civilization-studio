...
* Supports Spartan v9-style normal_icon_footprint / small_ui_safe_subject_scale for research-bar/build-tree icon sizing.
* Defaults can be reduced to 0.65625 to match the Spartan League v9 footprint.
...
class Config:
    ...
    # Optional JSON reference keys mirrored from SpartanLeague_art_reference_v9.json.
    # When set, these override the older building/promotion defaults for small UI
    # contexts such as the research bar and build/tech unlock rows.
    icon_fit_policy: str = "contain"
    normal_icon_footprint: Optional[float] = None
    small_ui_safe_subject_scale: Optional[float] = None
    research_bar_safe_subject_scale: Optional[float] = None
    build_tree_safe_subject_scale: Optional[float] = None
...
def parse_config(path: Path) -> Config:
    ...
    cfg.icon_fit_policy = data.get("icon_fit_policy", "contain")
    def _optional_float(*names):
        for name in names:
            if data.get(name) is not None:
                return float(data[name])
        return None
    cfg.normal_icon_footprint = _optional_float("normal_icon_footprint", "normal_icon_safe_subject_scale")
    cfg.small_ui_safe_subject_scale = _optional_float("small_ui_safe_subject_scale", "small_icon_safe_subject_scale")
    cfg.research_bar_safe_subject_scale = _optional_float("research_bar_safe_subject_scale", "research_bar_icon_safe_subject_scale")
    cfg.build_tree_safe_subject_scale = _optional_float("build_tree_safe_subject_scale", "build_tree_icon_safe_subject_scale")
...
def scale_for_icon(cfg: Config, icon: IconSpec) -> float:
    # Highest precedence: per-icon explicit value.
    if icon.safe_subject_scale is not None:
        return icon.safe_subject_scale

    # UDMB's icons appear in compressed UI contexts: tech picker/research bar
    # and build/tech unlock rows. These optional JSON keys intentionally mirror
    # the Spartan League v9 footprint convention, where 0.65625 makes a 256px
    # icon's visible art about 168px.
    small_ui_scale = (
        cfg.research_bar_safe_subject_scale
        or cfg.build_tree_safe_subject_scale
        or cfg.small_ui_safe_subject_scale
        or cfg.normal_icon_footprint
    )
    if small_ui_scale is not None:
        return float(small_ui_scale)
...
def build_atlas(cfg: Config, check_only: bool = False) -> dict:
    ...
    summary = {
        ...
        "icon_fit_policy": cfg.icon_fit_policy,
        "normal_icon_footprint": cfg.normal_icon_footprint,
        "small_ui_safe_subject_scale": cfg.small_ui_safe_subject_scale,
        "research_bar_safe_subject_scale": cfg.research_bar_safe_subject_scale,
        "build_tree_safe_subject_scale": cfg.build_tree_safe_subject_scale,
...
def write_report(cfg: Config, manifest: dict, path: Path) -> None:
    lines = [
        ...
        f"Icon fit policy: `{cfg.icon_fit_policy}`",
        f"Normal icon footprint: `{cfg.normal_icon_footprint}`",
        f"Small UI safe subject scale: `{cfg.small_ui_safe_subject_scale}`",
        f"Research bar safe subject scale: `{cfg.research_bar_safe_subject_scale}`",
        f"Build tree safe subject scale: `{cfg.build_tree_safe_subject_scale}`",
