...
UNIT_FLAG_GRID = 8
UNIT_FLAG_CELL = 32
# Spartan League v8/Firaxis-like footprint target used for generated circular icons.
# 168/256 = 0.65625, so a 256px atlas cell gets a 168px visible icon;
# 64px gets 42px; 32px gets 21px. This intentionally leaves more ring/margin
# than the earlier 0.92-0.95 max-fill configs, which looked oversized in Civ V.
STANDARD_CIV5_ICON_SCALE = 168 / 256
SAFE_OCCUPANCY = STANDARD_CIV5_ICON_SCALE  # fallback for standard circular icons; authoritative per-category values live in ICON_CATEGORY_PROFILES.
...
# ---------------------------------------------------------------------------
# AUTHORITATIVE per-category target mapping (the verified config checklist).
#   scale  = subject longest side / cell. The subject is CONTAINED at this
#            fraction of the cell and is never cover-filled, so an opaque /
#            full-bleed source can no longer blow up to fill the whole circle
#            (that cover behavior is what made icons larger than Firaxis stock).
#   mask   = the texture carries an alpha channel (True) or is fully opaque
#            (False). It selects the alpha capability of the DDS format; it is
#            NOT a baked circular cut-out -- at scale <= 0.78 the subject sits
#            well inside the inscribed circle and the game draws the ring frame.
#   format = DDS pixel format written to disk: A8R8G8B8 (uncompressed 32-bit
#            BGRA), DXT5 (BC3, alpha), DXT1_NOMIP (BC1, opaque, single surface),
#            DXT1_MIP (BC1, opaque, full mip chain).
# Raise these only after in-game comparison; 0.82+ was visibly too large in Civ V frames.
# ---------------------------------------------------------------------------
ICON_CATEGORY_PROFILES = {
    "BUILDING_AND_WONDER_ICONS": {"scale": STANDARD_CIV5_ICON_SCALE, "mask": True,  "format": "A8R8G8B8"},
    "UNIT_PORTRAIT_ICONS":       {"scale": STANDARD_CIV5_ICON_SCALE, "mask": True,  "format": "A8R8G8B8"},
    "PROMOTION_ICONS":           {"scale": STANDARD_CIV5_ICON_SCALE, "mask": True,  "format": "A8R8G8B8"},
    "LEADER_PORTRAIT_ICONS":     {"scale": STANDARD_CIV5_ICON_SCALE, "mask": True,  "format": "A8R8G8B8"},
    "UNIT_MAP_FLAGS":            {"scale": 0.78, "mask": True,  "format": "A8R8G8B8"},
    # Team-colour stencil: pure-white silhouette on transparent, recoloured by
    # the engine to the civ's colours. Keep this uncompressed A8R8G8B8/BGRA so
    # tiny 45px/32px glyphs keep crisp alpha and never collapse into a white square.
    "ALPHA_ICON_ATLAS":          {"scale": STANDARD_CIV5_ICON_SCALE, "mask": True,  "format": "A8R8G8B8"},
    "DAWN_OF_MAN_SCREEN":        {"scale": 1.00, "mask": False, "format": "A8R8G8B8"},
    "LEADER_2D_SCENE":           {"scale": 1.00, "mask": False, "format": "A8R8G8B8"},
}

# Pipeline role -> category. The civ emblem and unit portraits share the
# Spartan v8 normal atlas roles share the same 168/256 subject footprint;
# unit flags remain at their separate 0.78 glyph target. Scene roles map to the
# two full-bleed scene categories.
ROLE_TO_CATEGORY = {
    "civ": "UNIT_PORTRAIT_ICONS",
    "unit": "UNIT_PORTRAIT_ICONS",
    "building": "BUILDING_AND_WONDER_ICONS",
    "support": "PROMOTION_ICONS",
    "promotion": "PROMOTION_ICONS",
    "trait": "PROMOTION_ICONS",
    "dummy": "PROMOTION_ICONS",
    "leader": "LEADER_PORTRAIT_ICONS",
    "unit_flag": "UNIT_MAP_FLAGS",
    "alpha": "ALPHA_ICON_ATLAS",
    "alpha_icon": "ALPHA_ICON_ATLAS",
    "dawn_image": "DAWN_OF_MAN_SCREEN",
    "leader_scene": "LEADER_2D_SCENE",
}

# Subject scale per role, DERIVED from the category mapping (single source of
# truth) so every role's occupancy comes from ICON_CATEGORY_PROFILES.
ROLE_SAFE_SUBJECT_SCALE = {
    role: ICON_CATEGORY_PROFILES[cat]["scale"]
    for role, cat in ROLE_TO_CATEGORY.items()
    if cat in ICON_CATEGORY_PROFILES
}


def category_for_role(role: str) -> Optional[str]:
    """Return the ICON_CATEGORY_PROFILES key for a pipeline role, or None."""
    return ROLE_TO_CATEGORY.get(role)


def dds_format_for_role(role: str, default: str = "A8R8G8B8") -> str:
    """Return the target DDS pixel format for a role from the category mapping."""
    cat = ROLE_TO_CATEGORY.get(role)
    if cat and cat in ICON_CATEGORY_PROFILES:
        return ICON_CATEGORY_PROFILES[cat]["format"]
    return default
...
PROFILES: dict[str, dict] = {
    ...
    "civ5_alpha_glyph_black_to_transparent": {
        ...
        "force_white": True,
        "force_opaque": False,
        "safe_subject_scale": STANDARD_CIV5_ICON_SCALE,
...
class MedallionOptions:
    bake: bool = False
    # Baked-medallion proportions aligned to the verified vanilla standard-icon
    # occupancy (0.82). radius_percent 0.46 keeps a thin margin for the rim and
    # antialiasing, matching stock framing. (Medallion baking is OFF by default;
    # normal atlases are unbaked and let Civ V apply the circular UI crop.)
    fill_percent: float = 0.82
    radius_percent: float = 0.46
    rim: bool = False
...
class PipelineConfig:
    ...
    icon_fit_policy: str = "contain"        # auto | contain | cover -- auto and contain both scale the subject to the per-category target; only explicit cover fills the circle (legacy)
...
def _dxt_block_data(level_img: Image.Image, dxt: str) -> bytes:
    """Encode one surface to a legacy DDS via Pillow and return only the BC
    block payload (everything after the 128-byte header)."""
    bio = io.BytesIO()
    level_img.save(bio, format="DDS", pixel_format=dxt)
    return bio.getvalue()[128:]


def _save_dxt1_with_mips(img: Image.Image, path: Path) -> int:
    """Write an opaque DXT1 (BC1) DDS with a full mip chain.

    Pillow's DDS encoder ignores its ``mipmaps`` argument (it always emits a
    single surface), so the chain is built by hand: each level is halved with
    LANCZOS down to 1x1, encoded to BC1, and its block payload concatenated
    after a level-0 header patched with the mip count and MIPMAP/COMPLEX caps.
    Returns the number of mip levels written.
    """
    base = Image.alpha_composite(
        Image.new("RGBA", img.size, (0, 0, 0, 255)), img.convert("RGBA")
    ).convert("RGB")
    levels = []
    cur = base
    while True:
    ...
    bio = io.BytesIO()
    base.save(bio, format="DDS", pixel_format="DXT1")
    hdr = bytearray(bio.getvalue()[:128])
    flags = struct.unpack_from("<I", hdr, 8)[0] | 0x20000          # DDSD_MIPMAPCOUNT
    struct.pack_into("<I", hdr, 8, flags)
    struct.pack_into("<I", hdr, 28, len(levels))                   # dwMipMapCount
    caps = struct.unpack_from("<I", hdr, 108)[0] | 0x8 | 0x400000  # COMPLEX | MIPMAP
    struct.pack_into("<I", hdr, 108, caps)
    payload = bytes(hdr) + b"".join(_dxt_block_data(lv, "DXT1") for lv in levels)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(payload)
    return len(levels)


def write_dds(img: Image.Image, path: Path, fmt: str = "A8R8G8B8") -> str:
    """Write a Civ-V-compatible (legacy DX9 header) DDS in the given format.

    A8R8G8B8  -> uncompressed 32-bit BGRA (alpha, one surface) via save_as_dds.
    DXT5      -> BC3 with alpha (one surface).
    DXT1_NOMIP-> BC1 opaque (subject composited over black), one surface.
    DXT1_MIP  -> BC1 opaque with a generated mip chain.

    Block-compressed formats require width and height that are multiples of 4
    (D3D9 cannot create a BC texture otherwise), so any DXT request on a tile
    whose dimensions are not multiples of 4 transparently falls back to the
    lossless uncompressed A8R8G8B8 form. Returns the format actually written so
    the caller can validate/report it. No texconv or network access is required.
    """
    fmt = (fmt or "A8R8G8B8").upper()
    path.parent.mkdir(parents=True, exist_ok=True)
    img = img.convert("RGBA")
    w, h = img.size
    if fmt in ("DXT5", "DXT1_NOMIP", "DXT1_MIP") and (w % 4 or h % 4):
        save_as_dds(img, path)
        return "A8R8G8B8"
    if fmt == "A8R8G8B8":
        save_as_dds(img, path)
        return "A8R8G8B8"
    if fmt == "DXT5":
        scrub_transparent_rgb(img).save(path, format="DDS", pixel_format="DXT5")
        return "DXT5"
    if fmt == "DXT1_NOMIP":
        flat = Image.alpha_composite(
            Image.new("RGBA", img.size, (0, 0, 0, 255)), img).convert("RGB")
        flat.save(path, format="DDS", pixel_format="DXT1")
        return "DXT1_NOMIP"
    if fmt == "DXT1_MIP":
        _save_dxt1_with_mips(img, path)
        return "DXT1_MIP"
    save_as_dds(img, path)
    return "A8R8G8B8"
...
def fit_icon_master(
    ...
    fit_policy: str = "auto",
    cover_dense_threshold: float = 0.70,
    force_circular_alpha_cap: bool = False,
) -> tuple[Image.Image, dict]:
    """Fit arbitrary source art into a square icon master so the inscribed Civ V
    circle is always filled correctly -- never blank, never oversized.

    Two strategies, chosen per source:

      CONTAIN  Scale the discrete subject so its longest side == occupancy *
               master_size, centered on transparent padding. This is the strict
               vanilla path for glyph/emblem art with intended breathing room
               (the verified 0.80-0.85 occupancy). Transparent margin here is the
               correct vanilla look, NOT broken blank space.

      COVER    Scale the content so it covers the entire inscribed circle
               (shortest dimension == master_size), center, and crop the overflow.
               The game's circular mask trims the corners. This guarantees a full
               circle for full-bleed SQUARE images and for disc/CIRCULAR sources,
               which would otherwise leave gaps or a transparent ring under
               CONTAIN.

    fit_policy:
      "auto"     pick COVER when the cropped subject fills its own bounding box
                 densely (a solid square or a disc) or the source is fully
                 opaque/full-bleed; otherwise CONTAIN. (default)
      "contain"  always CONTAIN at the occupancy target.
      "cover"    always COVER.

    Returns (master_image, info). Alpha/team icons and unit flags do NOT use this
    fitter -- they are kept strictly CONTAIN at their fragile ratios elsewhere.
    """
    occ = SAFE_OCCUPANCY if occupancy is None else float(occupancy)
    label = source.name if isinstance(source, Path) else f"<{role}>"
    img = open_rgba(source) if isinstance(source, Path) else source.convert("RGBA")
    if force_circular_alpha_cap:
        img = apply_inscribed_ellipse_alpha(img)
        warnings.append(f"INFO: {role}:{label}: pre-scale circular alpha cap applied by config option.")
    if img.width != img.height:
        warnings.append(f"{role}:{label}: non-square source was fitted into a square master.")
    ...
    # Strip flat/edge-connected backgrounds so the alpha bbox is meaningful
    # (mirrors normalize_icon_master). Fully-painted sources then get a safe
    # circular alpha cap before fitting so square opaque support/promotion/unit
    # cells cannot bleed through Civ V's round UI frames.
    if fully_opaque:
        cleaned = remove_edge_connected_flat_background(img, f"{role}:{label}", warnings)
        if cleaned.getchannel("A").getextrema()[0] < 250:
            cleaned = prune_alpha_components(cleaned, f"{role}:{label}", warnings)
            img = cleaned
            a = img.getchannel("A")
            amin, _ = a.getextrema()
            fully_opaque = amin >= 250
    ...
    if fully_opaque:
        ...
        fully_opaque = False
        warnings.append(
            f"{role}:{label}: source remained fully opaque; applied circular alpha cap before fitting "
            "to prevent square icon bleed."
        )

    bbox = alpha_bbox(img) if not fully_opaque else None
    if bbox:
        subject = img.crop(bbox)
    else:
        subject = center_crop_to_square(img)
        bbox = (0, 0, src_w, src_h)
    sub_w, sub_h = subject.size

    # Density of the subject within its own bbox: solid square ~1.0, disc ~0.785,
    # sparse glyph well below. Drives the auto cover/contain decision.
    sa = subject.getchannel("A")
    opaque_px = sum(sa.histogram()[128:])  # alpha >= 128
    opaque_fraction = opaque_px / max(1, sub_w * sub_h)

    # Full-bleed: meaningful content reaches the source frame in both axes.
    bl, bt, br, bb = bbox
    touch = 0.95
    full_bleed = (br - bl) >= src_w * touch and (bb - bt) >= src_h * touch
    aspect = max(sub_w, sub_h) / max(1, min(sub_w, sub_h))
    squareish = aspect <= 1.30
    dense = opaque_fraction >= cover_dense_threshold

    policy = (fit_policy or "auto").lower()
    if policy not in {"auto", "contain", "cover"}:
        policy = "auto"
    if policy == "auto":
        # COVER (fill the circle, crop overflow) only for art that occupies a
        # roughly SQUARE footprint AND is solid -- a disc, medallion, or full
        # image -- or is full-bleed / fully opaque. Sparse or elongated glyphs
        # (swords, stars, animal silhouettes) stay CONTAIN at the vanilla
        # occupancy so they are never oversized.
        mode = "cover" if (fully_opaque or full_bleed or (squareish and dense)) else "contain"
    else:
        mode = policy

    canvas = Image.new("RGBA", (master_size, master_size), (0, 0, 0, 0))
    cover_cropped_opaque = False
    if mode == "cover":
        # Shorter dimension covers the full circle diameter (== master_size),
        # then center and crop the overflow; the circular UI mask trims corners.
        scale = master_size / min(sub_w, sub_h)
        nw, nh = max(1, round(sub_w * scale)), max(1, round(sub_h * scale))
        scaled = subject.resize((nw, nh), Image.Resampling.LANCZOS)
        left, top = (nw - master_size) // 2, (nh - master_size) // 2
        scaled = scaled.crop((left, top, left + master_size, top + master_size))
        canvas.alpha_composite(scaled)
        cover_cropped_opaque = (nw > master_size or nh > master_size) and opaque_fraction > 0.85
        circle_fill = 1.0
    else:  # contain
        max_subject = max(1, int(round(master_size * occ)))
        scale = min(max_subject / sub_w, max_subject / sub_h)
        nw, nh = max(1, round(sub_w * scale)), max(1, round(sub_h * scale))
        scaled = subject.resize((nw, nh), Image.Resampling.LANCZOS)
        canvas.paste(scaled, ((master_size - nw) // 2, (master_size - nh) // 2), scaled)
        circle_fill = max(nw, nh) / master_size

    canvas = scrub_transparent_rgb(canvas)

    note = f"INFO: {role}:{label}: fit={mode} (subject covers ~{circle_fill * 100:.0f}% of the circle)."
    if mode == "cover" and cover_cropped_opaque:
        note += " Full-bleed source: outer corners are cropped by the circular frame (expected)."
    elif mode == "contain" and circle_fill < occ - 0.05:
        note += " Source content is sparse; transparent margin inside the circle is intended (vanilla glyph spacing)."
    warnings.append(note)

    info = {
        "fit_mode": mode,
        "fit_policy": policy,
        "occupancy_target": round(occ, 3),
        "opaque_fraction": round(opaque_fraction, 3),
        "aspect": round(aspect, 2),
        "squareish": bool(squareish),
        "full_bleed_source": bool(full_bleed or fully_opaque),
        "circle_fill_estimate": round(circle_fill, 3),
        "cover_cropped_opaque": bool(cover_cropped_opaque),
    }
    return canvas, info
...
def prepare_alpha_icon(src_path: Path, warnings: list[str],
    ...
    img = normalize_icon_master_from_image(
        ...
        safe_subject_scale=safe_subject_scale_for_role("alpha", profile, safe_subject_scale),
    )
...
def _edge_luma_fraction(img: Image.Image, threshold: int = 12, band_pct: float = 0.03) -> float:
    """Return the fraction of border-band pixels at or below a luma threshold.

    This is used to distinguish a true flattened black matte around an already
    circular icon from an ordinary dark painting with smoky corners. The old
    pipeline only looked at the four corner pixels; that was too aggressive for
    dark Civ V art and could erase helmets, banners, bows, and shields.
    """
    rgb = img.convert("RGB")
    w, h = rgb.size
    band = max(1, int(round(min(w, h) * float(band_pct))))
    total = 0
    dark = 0
    px = rgb.load()
    ...
    def visit(x: int, y: int) -> None:
        nonlocal total, dark
        total += 1
        if _luma(px[x, y]) <= threshold:
            dark += 1

    for y in range(h):
        for x in range(band):
            visit(x, y)
        for x in range(max(band, w - band), w):
            visit(x, y)
    for y in range(band):
        for x in range(band, max(band, w - band)):
            visit(x, y)
    for y in range(max(band, h - band), h):
        for x in range(band, max(band, w - band)):
            visit(x, y)
    return dark / max(1, total)


def apply_inscribed_ellipse_alpha(img: Image.Image) -> Image.Image:
    """Make only the outside of the largest inscribed ellipse transparent.

    This is the safe path for AI-generated icon PNGs that already contain a
    circular painting on a black square. It removes the black corner matte without
    flood-filling into legitimate dark artwork inside the icon.
    """
    src = img.convert("RGBA")
    w, h = src.size
    scale = 4
    ...
    pad = max(0.5, min(w, h) * 0.001) * scale
...
def remove_edge_connected_flat_background(img: Image.Image, role: str, warnings: list[str],
    ...
    # The previous max-fill build treated any very dark corner set as a black
    # source matte and flood-filled it with a high tolerance. Dark Civ V art often
    # has smoky, low-luma corners; flood-filling those pixels erased real subject
    # pieces from the atlas. A true flattened black icon matte must have nearly
    # black corners AND a mostly black border band.
    edge_dark_fraction = _edge_luma_fraction(src, threshold=12, band_pct=0.03)
    is_dark_mat = avg_luma <= 12 and spread <= 12 and edge_dark_fraction >= 0.70
    ...
    # For black mattes on generated circular icons, do NOT flood-fill by color.
    # Flood-fill can leak into black crosses, chainmail, smoky skies, and weapon
    # silhouettes. A geometric ellipse removes only the outside-corner matte.
    if is_dark_mat:
        out = apply_inscribed_ellipse_alpha(src)
        removed_alpha = ImageChops.subtract(src.getchannel("A"), out.getchannel("A"))
        count = ImageStat.Stat(removed_alpha).sum[0] / 255
        warnings.append(
            f"{role}: removed black corner matte with safe ellipse mask "
            f"({count / max(1, w*h):.1%} of pixels; edge-dark {edge_dark_fraction:.1%})."
        )
        return out
    ...
    # Do not blindly erase whole corner rectangles. The previous implementation
    # attempted to remove AI watermark/sparkle residue this way, but it also cut
    # legitimate icon art out of dark, high-contrast paintings. Detached artifacts
    # are handled by prune_alpha_components() for small sources; large source art
    # is left intact rather than destructively altered.
    out.putalpha(a)
...
def prepare_unit_flag(src_path: Path, label: str, warnings: list[str],
    ...
    if assessment["painted"]:
        ...
        if allow_painted:
            ...
            return None, False
    flag_scale = safe_subject_scale_for_role("unit_flag", profile, safe_subject_scale)
    img = normalize_icon_master(src_path, f"unit_flag:{label}", warnings, safe_subject_scale=flag_scale)
    alpha = img.getchannel("A")
    white = Image.new("RGBA", img.size, (255, 255, 255, 0))
    white.putalpha(alpha)
...
def validate_config(cfg: PipelineConfig) -> list[str]:
    ...
    if cfg.icon_fit_policy not in {"auto", "contain", "cover"}:
        errors.append("icon_fit_policy must be auto, contain, or cover")
...
def make_template(prefix: str, output: Optional[Path], units: int, buildings: int,
    ...
    config: dict = {
        "_notes": [
            ...
            "Default normal/alpha icon footprint is 0.65625 of each cell, matching the Spartan League v8 icon scale pass.",
            "For any opaque square source that still shows corner bleed, add options: {\"pre_scale_circular_alpha_cap\": true} to that asset.",
        ],
        "prefix": prefix,
...
def generate_assets(cfg: PipelineConfig, log: Callable[[str], None] = print) -> dict:
    ...
    for item in normal_items:
        ...
        # Role-aware occupancy is applied for BOTH legacy and profiled configs so
        # the verified vanilla proportions hold regardless of config style.
        occ = safe_subject_scale_for_role(item.role, prof if cfg.uses_profiles else None, item.safe_subject_scale)
        # Firaxis sizing is deterministic: scale the subject to the per-category
        # target and never cover-fill. Only an explicit "cover" policy fills the
        # circle (legacy); "auto" now behaves as "contain".
        eff_policy = "cover" if str(cfg.icon_fit_policy).lower() == "cover" else "contain"
        force_cap = (as_bool(item.options.get("pre_scale_circular_alpha_cap", False))
                     or as_bool(item.options.get("force_circular_alpha_cap", False))
                     or as_bool(item.options.get("circular_alpha_cap", False))) if item.options else False
        master, fit_info = fit_icon_master(item.source, item.role, warnings,
                                           master_size=master_size, occupancy=occ,
                                           fit_policy=eff_policy,
                                           force_circular_alpha_cap=force_cap)
        ...
        normal_metrics.append({"role": item.role, "label": item.label, "profile": prof,
                               "safe_subject_scale": occ, "fit": fit_info, "metrics": metrics})
        if want_per_asset:
    ...
    for size in NORMAL_ATLAS_SIZES:
        ...
        # Combined normal-icon atlas now matches the Spartan League v8 output:
        # uncompressed 32-bit BGRA/A8R8G8B8 with alpha and no generated mipmaps.
        # This preserves the soft transparent padding around the 0.65625 subject
        # footprint and avoids BC compression fringing at 45px/32px UI sizes.
        used_fmt = write_dds(atlas, dds_dir / filename, "A8R8G8B8")
        validate_dds_file(dds_dir / filename, size * NORMAL_GRID, size * NORMAL_GRID, True, qa, fmt=used_fmt)
    ...
    alpha_files = []
    alpha_target_fmt = dds_format_for_role("alpha_icon")
    for size in ALPHA_ICON_SIZES:
        ...
        used_fmt = write_dds(img, dds_dir / name, alpha_target_fmt)
        if used_fmt != alpha_target_fmt:
            warnings.append(f"alpha {name}: {size}px is not a multiple of 4, so it was written as {used_fmt} instead of {alpha_target_fmt} (BC compression needs mult-of-4 dimensions; alpha is preserved).")
        alpha_files.append(name)
        validate_dds_file(dds_dir / name, size, size, True, qa, fmt=used_fmt)
    ...
    if cfg.dawn_image:
        ...
        used_fmt = write_dds(img, dds_dir / large_names["dawn"], dds_format_for_role("dawn_image"))
        scene_files["dawn_of_man"] = large_names["dawn"]
        validate_dds_file(dds_dir / scene_files["dawn_of_man"], w, h, False, qa, fmt=used_fmt)
    ...
    if cfg.map_image:
        ...
        used_fmt = write_dds(img, dds_dir / large_names["map"], "A8R8G8B8")
        scene_files["map"] = large_names["map"]
        validate_dds_file(dds_dir / scene_files["map"], w, h, True, qa, fmt=used_fmt)
    ...
    if cfg.leader_scene:
        ...
        used_fmt = write_dds(img, dds_dir / large_names["leader_scene"], dds_format_for_role("leader_scene"))
        scene_files["leader_scene"] = large_names["leader_scene"]
        validate_dds_file(dds_dir / scene_files["leader_scene"], w, h, False, qa, fmt=used_fmt)
...
def validate_dds_file(path: Path, width: int, height: int, require_alpha: bool, qa: list[QAResult], fmt: str = "A8R8G8B8") -> None:
    fmt = (fmt or "A8R8G8B8").upper()
    result = QAResult(path.name)
    try:
        h = read_dds_header(path)
        result.metrics.update(h)
        result.metrics["expected_format"] = fmt
        ...
        if fmt == "A8R8G8B8":
            ...
            if h["fourcc"] != _FOURCC["DXT5"]:
                result.warnings.append("DDS is not DXT5/BC3 as expected")
        elif fmt in ("DXT1_NOMIP", "DXT1_MIP"):
            if h["fourcc"] != _FOURCC["DXT1"]:
                result.warnings.append("DDS is not DXT1/BC1 as expected")
            if fmt == "DXT1_MIP" and h["mipmaps"] < 1:
                result.warnings.append(f"DXT1_MIP expected a mip chain; mipmap count is {h['mipmaps']}")
...
def build_manifest(cfg: PipelineConfig, normal_items: list[AssetSpec], icon_atlas: str, icon_stem: str, alpha_files: list[str], flag_files: list[str], flag_atlas: str, scene_files: dict, warnings: list[str], qa: list[QAResult]) -> dict:
    return {
        ...
        "icon_category_profiles": ICON_CATEGORY_PROFILES,
        "role_to_category": ROLE_TO_CATEGORY,
        "standards": {
            "normal_icon_atlas": "4x4 grid, sizes 256/128/80/64/45/32, uncompressed A8R8G8B8/BGRA with alpha, no mipmaps; subject contained at the Spartan v8/Firaxis-like 168/256 footprint (0.65625) for civ/leader/unit/building/support/promotion icons",
            "alpha_team_icons": "1x1 atlas rows, white-on-transparent uncompressed A8R8G8B8 at 128/80/64/48/45/32/24; glyph contained at 0.65625",
            "unit_flags": "8x8 256x256 atlas with 32px cells, uncompressed A8R8G8B8, silhouette at 0.78",
            "large_images": "Dawn of Man, leader scene, and map are written as uncompressed A8R8G8B8/BGRA DDS with no generated mipmaps",
        },
        "atlases": {
        ...
        "normal_icons": [
            {"role": i.role, "label": i.label, "source": str(i.source), "atlas": i.atlas,
             ...
             "pre_scale_circular_alpha_cap": (as_bool(i.options.get("pre_scale_circular_alpha_cap", False))
                                               or as_bool(i.options.get("force_circular_alpha_cap", False))
                                               or as_bool(i.options.get("circular_alpha_cap", False))) if i.options else False,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--icon-fit-policy', choices=['auto', 'contain', 'cover'], default=absent, help="How normal icons are sized: 'contain' (default) scales the subject to the Spartan v8/Firaxis-like 168/256 target (0.65625) and leaves the intended Civ V frame margin; 'auto' behaves identically to contain; 'cover' is legacy and fills the whole circle, cropping overflow (will look larger than stock)")
    ...
    p.add_argument('--medallion-fill-percent', type=float, default=absent, help='How much of the medallion circle the source art occupies (default: 0.82, the verified vanilla standard-icon ratio)')
    p.add_argument('--medallion-radius-percent', type=float, default=absent, help='Medallion radius relative to square cell size (default: 0.46)')
...
def config_from_args(args: argparse.Namespace, data: Optional[dict] = None) -> PipelineConfig:
    ...
    cfg = PipelineConfig(
        ...
        medallion=MedallionOptions(
            ...
            fill_percent=float(cfg_value(args, data, 'medallion_fill_percent', 0.82)),
            radius_percent=float(cfg_value(args, data, 'medallion_radius_percent', 0.46)),
        ...
        icon_fit_policy=(cfg_value(args, data, 'icon_fit_policy', 'contain') or 'contain'),
    )
