...
UNIT_FLAG_GRID = 8
UNIT_FLAG_CELL = 32
SAFE_OCCUPANCY = 0.58  # Firaxis-matched fallback for standard circular icons (subject longest side / cell). Authoritative per-category values live in ICON_CATEGORY_PROFILES.
...
# Raise these only after re-measuring vanilla; 0.82 was visibly too large.
# ---------------------------------------------------------------------------
ICON_CATEGORY_PROFILES = {
    "BUILDING_AND_WONDER_ICONS": {"scale": 0.58, "mask": True,  "format": "DXT5"},
    "UNIT_PORTRAIT_ICONS":       {"scale": 0.58, "mask": True,  "format": "DXT5"},
    "PROMOTION_ICONS":           {"scale": 0.68, "mask": True,  "format": "DXT5"},
    "LEADER_PORTRAIT_ICONS":     {"scale": 0.75, "mask": True,  "format": "A8R8G8B8"},
    ...
    # the engine to the civ's colours. DXT5 keeps alpha; any tile whose pixel
    # dimensions are not multiples of 4 (the 45px alpha tile) auto-falls-back to
    # uncompressed A8R8G8B8 in write_dds, since BC textures require mult-of-4
    # dimensions. NEVER DXT1 here -- it would drop the alpha to a white square.
    "ALPHA_ICON_ATLAS":          {"scale": 0.62, "mask": True,  "format": "DXT5"},
    "DAWN_OF_MAN_SCREEN":        {"scale": 1.00, "mask": False, "format": "DXT1_NOMIP"},
    "LEADER_2D_SCENE":           {"scale": 1.00, "mask": False, "format": "DXT1_MIP"},
}

# Pipeline role -> category. The civ emblem and unit portraits share the
# standard 0.58 icon size; promotions / abilities use 0.68; leaders 0.75; flags
# 0.78. Scene roles map to the two full-bleed scene categories.
ROLE_TO_CATEGORY = {
...
def fit_icon_master(
    ...
    # (mirrors normalize_icon_master). Fully-painted sources stay opaque.
    if fully_opaque:
...
def generate_assets(cfg: PipelineConfig, log: Callable[[str], None] = print) -> dict:
    ...
    for size in NORMAL_ATLAS_SIZES:
        ...
        # Combined normal-icon atlas (civ/leader/unit/building/promotion share one
        # atlas, so it carries a single format): DXT5/BC3 with alpha. The leader
        # ICON inherits DXT5 here; its A8R8G8B8 preference only applies to a
        # standalone leader atlas, which this combined layout does not use.
        used_fmt = write_dds(atlas, dds_dir / filename, "DXT5")
    ...
    if cfg.dawn_image:
        ...
        used_fmt = write_dds(img, dds_dir / large_names["dawn"], "DXT1_NOMIP")
    ...
    if cfg.leader_scene:
        ...
        used_fmt = write_dds(img, dds_dir / large_names["leader_scene"], "DXT1_MIP")
...
def build_manifest(cfg: PipelineConfig, normal_items: list[AssetSpec], icon_atlas: str, icon_stem: str, alpha_files: list[str], flag_files: list[str], flag_atlas: str, scene_files: dict, warnings: list[str], qa: list[QAResult]) -> dict:
    return {
        ...
        "standards": {
            "normal_icon_atlas": "4x4 grid, sizes 256/128/80/64/45/32, DXT5/BC3 with alpha, no mipmaps; one combined atlas so the leader icon shares DXT5; subject contained at the per-category scale (0.58 unit/building, 0.68 promotion, 0.75 leader)",
            "alpha_team_icons": "FLORENCE_ALPHA_ATLAS-style 1x1 atlas rows, white-on-transparent uncompressed A8R8G8B8 at 128/80/64/48/45/32/24",
            "unit_flags": "8x8 256x256 atlas with 32px cells, uncompressed A8R8G8B8, silhouette at 0.78",
            "large_images": "Dawn of Man DXT1 (no mipmaps), Leader 2D scene DXT1 (with mip chain); map 512x512 uncompressed A8R8G8B8",
        },
        "atlases": {
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--icon-fit-policy', choices=['auto', 'contain', 'cover'], default=absent, help="How normal icons are sized: 'contain' (default) scales the subject to the per-category target (0.58 units/buildings, 0.68 promotions, 0.75 leaders) and leaves the intended margin; 'auto' behaves identically to contain; 'cover' is legacy and fills the whole circle, cropping overflow (will look larger than Firaxis stock)")
