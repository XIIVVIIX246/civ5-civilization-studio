...
* safe_subject_scale defaults to 0.84 for buildings and 0.86 for promotions.
* No baked medallions and no circular mask by default. This keeps tech-picker
  icons from appearing oversized compared with Firaxis assets.
...
class Config:
    ...
    default_safe_subject_scale: float = 0.84
    building_safe_subject_scale: float = 0.84
    promotion_safe_subject_scale: float = 0.86
...
def normalize_icon_master(src_path: Path, label: str, scale: float, master_size: int = 1024, warnings: Optional[list[str]] = None) -> Image.Image:
...
def make_tech_size_preview(masters: dict[int, tuple[IconSpec, Image.Image]], cfg: Config, path: Path) -> None:
    ...
    for pos, idx in enumerate(sorted(masters)):
        ...
        x = (pos % cols) * cell
        y = (pos // cols) * cell
        icon45 = master.resize((45, 45), Image.Resampling.LANCZOS)
...
def parse_config(path: Path) -> Config:
    ...
    cfg.default_safe_subject_scale = float(data.get("default_safe_subject_scale", 0.84))
    cfg.building_safe_subject_scale = float(data.get("building_safe_subject_scale", 0.84))
    cfg.promotion_safe_subject_scale = float(data.get("promotion_safe_subject_scale", 0.86))
...
def scale_for_icon(cfg: Config, icon: IconSpec) -> float:
    ...
    if role == "unitpromotions" or type_name.startswith("PROMOTION_"):
...
def build_atlas(cfg: Config, check_only: bool = False) -> dict:
    ...
    for icon in cfg.icons:
        ...
        master = normalize_icon_master(icon.source, f"{icon.portrait_index}:{icon.label}", scale, 1024, warnings)
