...
UNIT_FLAG_GRID = 8
UNIT_FLAG_CELL = 32
SAFE_OCCUPANCY = 0.82  # subject should fit inside center 80-84% (legacy global fallback).
...
def looks_like_white_on_black(src_path: Path) -> bool:
    ...
    bright = sum(1 for p in gray.getdata() if p >= 160) / (48 * 48)
...
def prepare_unit_flag(src_path: Path, label: str, warnings: list[str],
    ...
    """
    if profile == "civ5_unit_flag_black_to_transparent":
        original = open_rgba(src_path)
        scale = safe_subject_scale_for_role("unit_flag", profile, safe_subject_scale)
        cleaned = black_background_to_white_transparent_glyph(
            original, force_white=True, safe_subject_scale=scale,
            master_size=PROFILES[profile].get("intermediate_size", 256),
            warnings=warnings,
        )
...
def _planned_dds_names(cfg: PipelineConfig) -> list[str]:
    ...
    if cfg.flags:
        names.append(f"{flag_stem}_32.dds")
...
def generate_assets(cfg: PipelineConfig, log: Callable[[str], None] = print) -> dict:
    ...
    for item in normal_items:
        ...
        if not explicit and cfg.default_profile:
            cand = PROFILE_ALIASES.get(cfg.default_profile.strip(), cfg.default_profile.strip())
    ...
    if cfg.flags:
        generated_paths.append(dds_dir / f"{flag_stem}_32.dds")
    if cfg.dawn_image:
    ...
    if cfg.flags:
        ...
        for i, fl in enumerate(cfg.flags):
            ...
            fl.resolved_profile = resolve_asset_profile("unit_flag", fl.profile or None, fl.options) or ""
    ...
    write_atlas_xml(xml_dir / f"{cfg.prefix}_AtlasDefinitions.xml", icon_selection, alpha_selection, flag_atlas if cfg.flags else "", flag_stem)
...
def write_atlas_xml(path: Path, icon_selection: AtlasSelection, alpha_selection: AtlasSelection, flag_atlas: str, flag_stem: str) -> None:
    ...
    if flag_atlas:
        lines += ['    <Row>', f'      <Atlas>{flag_atlas}</Atlas>', '      <IconSize>32</IconSize>', f'      <Filename>{flag_stem}_32.dds</Filename>', '      <IconsPerRow>8</IconsPerRow>', '      <IconsPerColumn>8</IconsPerColumn>', '    </Row>']
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--medallion-fill-percent', type=float, default=absent, help='How much of the medallion circle source art occupies (default: 0.84)')
    ...
    p.add_argument('--medallion-rim-width-percent', type=float, default=absent, help='Rim width relative to square cell size (default: 0.035)')
...
def config_from_args(args: argparse.Namespace, data: Optional[dict] = None) -> PipelineConfig:
    ...
    for spec in units + buildings + supports + flags:
    ...
    cfg = PipelineConfig(
        ...
        medallion=MedallionOptions(
            ...
            fill_percent=float(cfg_value(args, data, 'medallion_fill_percent', 0.84)),
            ...
            rim_width_percent=float(cfg_value(args, data, 'medallion_rim_width_percent', 0.035)),
