...
def build_normal_icon_cell(icon: IconItem, path: Path, size: int) -> Image.Image:
    ...
    if profile == "raw_square_medallion":
        ...
        return build_raw_square_medallion(img, size, subject_ratio)
    raise ValueError(f"Unknown normal icon profile: {icon.profile}")


def build_alpha_cell(path: Path, size: int, visible_ratio: float = ALPHA_VISIBLE_RATIO) -> Image.Image:
    img = load_rgba(path)
    return fit_subject_square(img, size, visible_ratio)


def build_unit_flag_cell(path: Path, size: int = UNIT_FLAG_SLOT_SIZE, visible_ratio: float = UNIT_FLAG_VISIBLE_RATIO) -> Image.Image:
    img = load_rgba(path)
    return fit_subject_square(img, size, visible_ratio)
...
def build(cfg: BuilderConfig, config_path: Path) -> dict[str, list[Path]]:
    ...
    if normal_items:
        for size in NORMAL_ATLAS_SIZES:
            ...
            dds_path = out_dir / f"{cfg.output.normal_atlas_base}_{size}.dds"
            write_dds_rgba(atlas, dds_path)
            outputs["dds"].append(dds_path)

            preview_path = preview_dir / f"{cfg.output.normal_atlas_base}_{size}_preview.png"
    ...
    if cfg.assets.alpha_icon:
        ...
        for size in ALPHA_SIZES:
            img = build_alpha_cell(alpha_file, size, alpha_ratio)
            dds_path = out_dir / f"{cfg.output.alpha_base}_{size}.dds"
            write_dds_rgba(img, dds_path)
            outputs["dds"].append(dds_path)

            preview_path = preview_dir / f"{cfg.output.alpha_base}_{size}_preview.png"
    ...
    if cfg.assets.unit_flags:
        ...
        for item in cfg.assets.unit_flags:
            ...
            flag_cells.append(build_unit_flag_cell(resolve_path(base_dir, item["file"]), UNIT_FLAG_SLOT_SIZE, ratio))
